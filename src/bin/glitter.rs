...
fn main() {
    ...
    let input_path = options.value_of("INPUT").expect("Missing required parameter INPUT").to_string();
    let output_path = options.value_of("OUTPUT").expect("Missing required parameter OUTPUT").to_string();
    ...
    let mut input_reader : Box<dyn BufRead> = if input_path == "-" {
        ...
        starting_directory = if let Some(parent_dir) = path.parent() {
            parent_dir.to_str().expect("No clue how this can happen that there is a file without any parent directory").to_owned()
    ...
    if let Err(error) = process(&mut input_reader, filename, starting_directory, &mut output_writer) {
        eprintln!("{}", error);
