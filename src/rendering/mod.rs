...
pub const DEFAULT_RENDERER_CONFIG : TemplateRendererConfig = TemplateRendererConfig { marker: '*', start_block: '{', end_block: '}', line_quote: '>' };
...
impl<'a> TemplateRenderer<'a> {
    pub fn new(output: &'a mut dyn Write) -> Self {
        Self { config: DEFAULT_RENDERER_CONFIG, output}
    }
    ...
    pub(crate) fn render<TValueRenderer: ValueRenderer>(&mut self, template: &TemplateDefinition, body_iterations: usize, context: Rc<ProcessingContext>, value_renderer: &TValueRenderer) -> Result<(), TemplateRenderError> {
    ...
    fn heavylift_render<TValueRenderer: ValueRenderer>(&mut self, template: &TemplateValue, iteration_count: usize, context: &Rc<ProcessingContext>, value_renderer: &TValueRenderer) -> Result<(), TemplateRenderError> {
            ...
            if chars[char_index] == self.config.marker {
                match self.is_processing_block(&chars, char_index) {
                    ProcessingStatement::Block => {
                        ...
                        let (var_path, block_length) = self.extract_enclosed_var_path(&template_content, &chars, char_index + 2, self.config.end_block)?;
                        value_renderer.render_value(&var_path, iteration_count, self).context(ValueGenerationError)?;
                    ...
                    ProcessingStatement::LineQuote => {
                        ...
                        let (var_path, line_length) = self.extract_trailing_var_path(&template_content, &chars, char_index + 2)?;
                        value_renderer.render_value(&var_path, iteration_count, self).context(ValueGenerationError)?;
    ...
    fn read_template(&self, template: &TemplateValue, context: &Rc<ProcessingContext> ) -> Result<String, TemplateRenderError> {
        match template {
            ...
            TemplateValue::Quote(file_path) => {
                ...
                let file = std::fs::File::open(full_file_path).context(InvalidTemplateFile {file_name: file_path})?;

                let mut buf_reader = BufReader::new(file);
                let mut contents = String::new();
                buf_reader.read_to_string(&mut contents).context(InvalidTemplateFile {file_name: file_path})?;
    ...
    fn quote(&mut self, template: &str, from: usize, to_option: Option<usize>) -> Result<(), TemplateRenderError> {
        if let Some(to) = to_option {
            self.output.write(template[from..=to].as_bytes()).context(OutputWriteError)?;
        }        

        Ok(())
    }

    fn extract_enclosed_var_path(&self, template: &str, chars: &Vec<char>, start_position: usize, end_marker: char) -> Result<(String, usize), TemplateRenderError> {
        ...
        if !hit_end {
            NonTerminatedProcessingBlock { start_at: start_position }.fail()

        } else if end == start_position {
            EmptyProcessingBlock {start_at: start_position}.fail()    

        } else {
            Ok((template[start_position..=end].trim().to_owned(), end - start_position + 1))
        }
    }

    fn extract_trailing_var_path(&self, template: &str, chars: &Vec<char>, start_position: usize) -> Result<(String, usize), TemplateRenderError> {
        let mut end = start_position;
