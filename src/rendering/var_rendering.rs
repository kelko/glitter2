...
use crate::{
    ...
    rendering::{TemplateRenderer, ValueRenderError, WriteValueError, InvalidCalculateCall},
...
impl RenderableVariable for RenderableRawValue {
    fn render(&self, output: &mut TemplateRenderer) -> std::result::Result<(), ValueRenderError> { 
        match &self.value {
            ...
            RawValue::Integer(value) => output.write(format!("{}", value).as_bytes()).context(WriteValueError)?,
            RawValue::Float(value) | RawValue::String(value) => output.write(value.as_bytes()).context(WriteValueError)?
        };
...
impl RenderableVariable for RenderableQuote {
    fn render(&self, output: &mut TemplateRenderer) -> std::result::Result<(), ValueRenderError> { 
        ...
        for line in buffered.lines() {
            output.write(line.context(WriteValueError)?.as_bytes()).context(WriteValueError)?;
    ...
    fn calculate(&self) -> std::result::Result<RawValue, ValueRenderError> { 
        ...
        buffered.read_to_string(&mut result).context(WriteValueError)?;
...
impl RenderableVariable for SubRender {
    fn render(&self, renderer: &mut TemplateRenderer) -> std::result::Result<(), ValueRenderError> { 
        if let Err(error) = self.processor.render(renderer) {
            return Err( ValueRenderError::SubRenderError{ source: Box::new(error) });
        }
